...
model{ 
  ...
  for(t in 1:NTrials){ 
   ...
   zVals[1] = (pow(S[t,1] / exp(sigma[1] + kappa_sig[1] * S[t,2]), 
     exp(beta[1])) + kappa_mu[1] * S[t,2]) / 1.414214;
   zVals[2] = (pow(S[t,2] / exp(sigma[2] + kappa_sig[2] * S[t,1]), 
     exp(beta[2])) + kappa_mu[2] * S[t,1]) / 1.414214;
